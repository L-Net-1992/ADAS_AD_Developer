...
void MonitorDialog::ReplayTabCreatNewChart(QString name)
{
    ...
    MonitorChartView * chartview = new MonitorChartView(this);
...
//    chart->setTheme(QChart::ChartThemeBrownSand);   // 主题颜色
    ...
    for(int i=0;i<replay.GetSignalName().size();i++) {
        QLineSeries *series = new QLineSeries(this);
        series->setName(replay.GetSignalName().at(i));
...
void MonitorDialog::timeoutSlotTimer3()
{
    ...
    if(x_index1_>xMax) {
        auto dis = x_index1_-xMax;
        m_chartview->chart()->scroll(dis,0);
    }
}
...
void MonitorDialog::on_btn_device_connect_clicked()
{
    ...
    {      
        ...
        QObject::connect(inspector_, &Inspector::varUpdated, this, [=](const QString & name, float value){
            if(monitor_running_) {
                QPointF data(x_index1_,value);
                QString str = name;
                monitor_.SendSignalData(str, data);

                if(data.y() > y_val_range1_.at(0)) y_val_range1_[0] = data.y(); // max
                if(data.y() < y_val_range1_.at(1)) y_val_range1_[1] = data.y(); // min
            }
        });
        ...
        if(var_state.size()) {
           ...
           ui->btn_device_connect->setText("断开");
           // 使能按键功能 tab1
...
void MonitorDialog::on_btn_replay_open_clicked()
{
    ...
    if(ret) {
        ...
        x_index_ = 0;
        for(auto it=replay_chart_.begin();it!=replay_chart_.end();it++) {
            replay_chart_.value(it.key())->axisX()->setRange(0, 50);
        }
        ...
        for(size_t i=0; i<signalname.size();i++) {
           ...
           replay.SetSignalDataSet(QString::fromStdString(signalname.at(i)), tmp);
        }
        file.close();
        replay_running_=1;
    }
...
void MonitorDialog::on_btn_monitor_start_clicked()
{
    ...
    m_chartview->chart()->axisX()->setRange(0,500);
...
{        
    if((monitor_running_ == 1) && (!record_running_)) {
        ...
        QString start_time  = QString::number(QDateTime::currentMSecsSinceEpoch());
        qDebug() << "start_time: "<< start_time.toULong();
    }
